...
| `lsf_cluster_confirmation` | If you have confirmed the availability of the {{site.data.keyword.spectrum_short}} license for a production cluster on {{site.data.keyword.cloud_notm}} or if you are deploying a non-production cluster, enter `true`. **Note:** Failure to comply with licenses for production use of software is a violation of the [IBM International Program License Agreement](https://www.ibm.com/software/passportadvantage/programlicense.html){: external} | Yes | None |
